import { PREFIX, DATA_SCENA_ELEMENT_ID } from "./const";
import type { ScenaFunctionComponent, ScenaProps, ScenaComponent, ScenaJSXElement, ScenaFunctionJSXElement, IObject } from "./types";
...
export function getTargets(ids: string[]) {
    return ids.map(id => getTarget(id)!);
}
