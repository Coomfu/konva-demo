...
export function uploadFileAndGetLocalUrl(): Promise<{ imgSrc: string, image: HTMLImageElement }> {
...
export const getLayerData = (
  ...
  return new Promise<Blob>((resolve, reject) => {
    const cv = window.cv;
    if (!cv) {
      return reject(new Error("OpenCV 未加载"));
    }
    ...
    try {
      ...
      const src = cv.imread(image); // 读取为 BGR
      const rect = new cv.Rect(cropX, cropY, cropWidth, cropHeight);
      const cropped = src.roi(rect); // 裁剪
